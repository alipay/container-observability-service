...
func (handler *PodInfoHandler) QueryPodInfoWithPodUid(key, value string) (int, interface{}, error) {
	...
	if len(podYamls) < 1 || len(podInfos) < 1 {
		return http.StatusOK, nil, nil
	}
	datafram := service.ConvertPodInfo2Frame(*podInfos[0], *podYamls[0])
	return http.StatusOK, datafram, nil

}

func (handler *PodInfoHandler) Process() (int, interface{}, error) {
	defer utils.IgnorePanic("PodInfoHandler.Process ")

	var result interface{}
	var err error
	var httpStatus int
	if handler.requestParams.PodUID != "" {
		httpStatus, result, err = handler.QueryPodInfoWithPodUid(handler.requestParams.PodUIDName, handler.requestParams.PodUID)
	}
